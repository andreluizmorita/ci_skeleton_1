...
if ( ! function_exists('barcode'))
{
	...
	function barcode($value, $largura = NULL, $altura = NULL, $esp_fina = NULL, $esp_grossa = NULL, $name = NULL) 
	{
		$CI =& get_instance();
		
		$pre = $CI->config->item('subclass_prefix');
		$CI->load->config($pre . 'assets');
		$CI->load->config($pre . 'barcode');		
		
		$largura 	= is_null($largura) ? $CI->config->item('barcode_largura') : $largura;
		$altura  	= is_null($altura) ? $CI->config->item('barcode_altura') : $altura;
		$esp_fina 	= is_null($esp_fina) ? $CI->config->item('barcode_esp_fina') : $esp_fina;
		$esp_grossa = is_null($esp_grossa) ? $CI->config->item('barcode_esp_grossa') : $esp_grossa;
		$name 		= is_null($name) ? $CI->config->item('barcode_name') : $name;
		
		$save_path = $CI->config->item('asset_path') . $CI->config->item('img_folder') . $CI->config->item('barcode_folder') . '/' .$name . '_' . $value . $CI->config->item('barcode_ext');
		
		$barcode_image = $CI->config->item('barcode_folder') . '/' .$name  . '_' . $value . $CI->config->item('barcode_ext');
		...
		$fino  = $esp_fina ;
		$largo = $esp_grossa; // Para o windows funciona mas o tamanho ideal seria 3 ou 2.72
		$pos   = 5 ;	
		
		// Largura da area do codigo de barra
		$lw = (($largo * strlen($value) * strlen($value)) + ($largo * 5)) / 2 - (strlen($value) * $largura); 
	
		// Altura do codigo de barra
		$hi = $altura;
		...
		*/
      	$img = imagecreate($lw, $hi);
      
      	$preto  = imagecolorallocate($img, 0, 0, 0);
      	$branco = imagecolorallocate($img, 255, 255, 255);
 		
      	imagecolortransparent ($img, $branco);

		// Cria o retángulo principal onde a imagem será aplicada.
      	imagefilledrectangle($img, 0, 0, $lw, $hi, $branco);
      
		// Abertura do código de barras.
      	imagefilledrectangle($img, 1,0,1,65,$preto);
      	imagefilledrectangle($img, 2,0,2,65,$branco);
      	imagefilledrectangle($img, 3,0,3,65,$preto);
      	imagefilledrectangle($img, 4,0,4,65,$branco);
	  
		// Atribui o valor para a variavel texto
		$texto = $value;
		if((strlen($texto) % 2) <> 0)
		    $texto = $texto;
			...
			$f = $tabcodbarra[$i];
		  	...
		  	for($i=1;$i<11;$i+=2)
		  	{
				...
				// Imprimindo uma barra preta   
			  	imagefilledrectangle($img, $pos,0,$pos-1+$f1,65,$preto);
			  	$pos = $pos + $f1 ;  
				...
				// Imprimindo uma barra branca
				  imagefilledrectangle($img, $pos,0,$pos-1+$f2,65,$branco);
				  $pos = $pos + $f2 ;  
		...
		imagefilledrectangle($img, $pos,0,$pos-1+$largo,65,$preto);
		$pos=$pos+$largo;
		
		imagefilledrectangle($img, $pos,0,$pos-1+$fino,65,$branco);
		$pos=$pos+$fino;		
		
		imagefilledrectangle($img, $pos,0,$pos-1+$fino,65,$preto);
		$pos=$pos+$fino;
		
		if ( $CI->config->item('barcode_ext') == ".jpeg")
