...
class CI_DB_active_record extends CI_DB_driver {
	 ...
	 * @return	object
	 */
	public function like($field, $match = '', $side = 'both', $escaped = TRUE)
	{
		return $this->_like($field, $match, 'AND ', $side, $escaped);
	}
	 ...
	 * @return	object
	 */
	protected function _like($field, $match = '', $type = 'AND ', $side = 'both', $escaped = FALSE, $not = '')
	{
		if ( ! is_array($field))
		{
		...
		foreach ($field as $k => $v)
		{
            ...
            //--- André - alteração para possibilitar desativar o escape de caracteres especiais no LIKE
			if($escaped)
	 ...
	 * Get_Like
	 *
	 * Allows the like clause, limit and offset to be added directly
	 *
	 ...
	 * @return	object
	 */
	public function get_like($table = '', $like = null, $limit = null, $offset = null)
	{
		if ($table != '')
		{
			$this->from($table);
		...
		{
            if( (isset($like['field']) OR isset($like[0])) && ( isset($like['match']) OR isset($like[1]) ))
            {
                $field = isset($like['field'])?$like['field']:$like[0];
                $match = isset($like['match'])?$like['match']:$like[1];
                $side  = isset($like['side'])?$like['side']:isset($like[3])?$like[3]:'both';
                
                $this->like($field, $match, $side);
            }
		}
