...
/*
 * 1- Dividir em métodos de load por blocos header / main / class / method / footer / flashdata 
 * 
 */
...
class Layout_new
{
	...
	public function output( $method = NULL, $class = NULL)
	{
		...
		if( file_exists(  $this->config['assets'] . $this->config['js_layout']) )
		...
		if( file_exists(  $this->config['assets'] . $this->config['css_layout']) )
		...
		if( file_exists( APPPATH . 'views/' . $this->config['layout_main_page']) ):
			...
			elseif( file_exists( APPPATH . 'views/' . $this->config['main']) )
			{
				...
				if(isset($this->config['main_data'])):
                    $this->CI->parser->parse_string($this->CI->load->view( $this->config['layout_main'] , $this->config['layout_main_data'], TRUE), $layout);
                else:
